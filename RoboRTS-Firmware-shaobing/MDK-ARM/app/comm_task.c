...
//    else
    {
      switch (tx_packet_id)
      {
        ...
        case BOTTOM_VERSION_ID:
        {
          data_packet_pack(BOTTOM_VERSION_ID, (uint8_t *)&pc_send_mesg.version_info_data,
                                   sizeof(version_info_t), UP_REG_ID);
          tx_packet_id = CLIENT_TO_ROBOT_ID;
        }break;
  ...
  while (1)
  {
    ...
    if (event.status == osEventSignal)
    {
      //receive judge data puts fifo
			pc_send_mesg.student_custom_data.data[4] = 0x1;
      if (event.value.signals & JUDGE_UART_IDLE_SIGNAL)
      {
//				pc_send_mesg.student_custom_data.data[3] = 0x1;
      ...
      {
//				pc_send_mesg.student_custom_data.data[3] = 0x2;
      ...
      {
//				pc_send_mesg.student_custom_data.data[5] = 0x1;
        send_packed_fifo_data(&judge_txdata_fifo, DN_REG_ID);
      }
      
      if (unpack_flag)
      {
//				pc_send_mesg.student_custom_data.data[3] = 0x3;
...
void data_packet_pack(uint16_t cmd_id, uint8_t *p_data, uint16_t len, uint8_t sof) //pack data for sending to pc
{
  ...
  /* use mutex operation */ //may add another sof
  if (sof == UP_REG_ID)
