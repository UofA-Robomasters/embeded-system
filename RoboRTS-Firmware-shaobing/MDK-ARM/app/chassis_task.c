...
//    get_chassis_mode();
  ...
  {
     //write_uart_noblocking(*huart6,*chassis.ctrl_mode,8);
		case DODGE_MODE:
    {
      chassis.vx = 0;
      chassis.vy = 0;
  ...
  for (int i = 0; i < 4; i++)
  {
	...
	pc_send_mesg.student_custom_data.data[4] = (uint8_t)judge_rece_mesg.power_heat_data.chassis_volt;
	pc_send_mesg.student_custom_data.data[4] = (uint8_t)judge_rece_mesg.power_heat_data.chassis_power;
...
void mecanum_calc(float vx, float vy, float vw, int16_t speed[])
{
  ...
  if (rotation_center_gimbal ==0 ) //used to == 1
  {
...
void chassis_param_init(void)
{
  ...
  PID_struct_init(&pid_chassis_angle, POSITION_PID, MAX_CHASSIS_VR_SPEED, 50, 14.0f, 0.0f, 50.0f); //pid used to 14 0 50
