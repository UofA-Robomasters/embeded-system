...
ChassisSpeed_Ref_t HeroSpeedRef;
//ChassisSpeed_Ref_t HeroTrackRef;
Gimbal_Ref_t HeroTrackRef;
Gimbal_Ref_t GimbalRef;
Gimbal_Ref_t HeroClawRef;
...
void RemoteDataPrcess(uint8_t *pData)
{
	...
	switch(GetInputMode())
	{
		...
		case STOP:
		{
			...
			HeroRemoteControlProcess(&(RC_CtrlData.rc));
			//紧急停车
		}break;
	}
}
...
/* hero remote control*/
void HeroRemoteControlProcess(Remote *rc)
{
			uint8_t a = 0x5;
			//USART_SendData(USART3, a);
			HeroSpeedRef.forward_back_ref = (RC_CtrlData.rc.ch1 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * STICK_TO_CHASSIS_SPEED_REF_FACT;
			//HeroClawRef.pitch_angle_dynamic_ref += (rc->ch3 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * STICK_TO_PITCH_ANGLE_INC_FACT;
			HeroTrackRef.pitch_angle_dynamic_ref += (rc->ch3 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * STICK_TO_CHASSIS_SPEED_REF_FACT;
...
{
	VAL_LIMIT(GimbalRef.pitch_angle_dynamic_ref, -PITCH_MAX+7, PITCH_MAX+2);
