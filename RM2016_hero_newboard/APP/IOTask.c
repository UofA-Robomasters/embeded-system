...
static uint32_t ManualCaliCmdFlagGrp = 1; //
//static uint32_t ManualCaliCmdFlagGrp = 0; //
...
void ResetManualCaliCmdFlag(void)
{
	ManualCaliCmdFlagGrp =0;
}
...
uint32_t ManualGetCaliCmdFlagGrp()
{
	return ManualCaliCmdFlagGrp;
}
...
bool ManualSetGimbal(void){
	...
	//else 
	//{
		while(loopCount++ < loopTime) {  //in cali state
			pitchSum += GMPitchEncoder.raw_value;
			yawSum += GMYawEncoder.raw_value;
		}
		GimbalCaliData.GimbalPitchOffset = pitchSum/loopTime;   //读取pitch轴陀螺仪作为偏差
...
bool ManualSetMag(void){
		int16_t x;
		int16_t y;
		int16_t z;
		HMC58X3_newValues(x,y,z); //read new mag value 			
	/*if(Is_Lost_Error_Set(LOST_ERROR_IMU))    
	{
		return false;
	}
	...
	{	
	*/
		MagCaliData.MagXOffset = x;
		MagCaliData.MagYOffset = y;
		MagCaliData.MagZOffset = z;
		MagCaliData.MagXScale = 1.0;
		MagCaliData.MagYScale = 1.0;
		MagCaliData.MagZScale = 1.0;
		return true;
