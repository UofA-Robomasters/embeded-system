...
volatile Encoder ShootMotorEncoder = {0,0,0,0,0,0,0,0,0};
volatile Encoder HeroReloadEncoder = {0,0,0,0,0,0,0,0,0};
volatile Encoder HeroTrackEncoder = {0,0,0,0,0,0,0,0,0};
		...
		switch(msg->StdId)
		{
				...
				case CAN_BUS1_MOTOR5_FEEDBACK_MSG_ID:
				{
					...
					if(GetWorkState() == PREPARE_STATE)   //准备阶段要求二者之间的差值一定不能大于阈值，否则肯定是出现了临界切换
					 {
							 if((GMYawEncoder.ecd_bias - GMYawEncoder.ecd_value) <-3000)
							 {
								GMYawEncoder.ecd_bias = gAppParamStruct.GimbalCaliData.GimbalYawOffset + 6192;
							 }
							 else if((GMYawEncoder.ecd_bias - GMYawEncoder.ecd_value) > 3000)
							 {
								GMYawEncoder.ecd_bias = gAppParamStruct.GimbalCaliData.GimbalYawOffset - 6192;
							 }
					 }
				...
				case CAN_BUS2_SHOOTMOTOR_MSG_ID:
				{
					//LostCounterFeed(GetLostCounter(LOST_COUNTER_INDEX_MOTOR4));
				 	(can_count<=50) ? GetEncoderBias(&ShootMotorEncoder ,msg):EncoderProcess(&ShootMotorEncoder ,msg);
					}break;
				default:
				{
				}
...
{      
		...
		switch(msg->StdId)
		{
				case CAN_BUS2_HEROARMH1_FEEDBACK_MSG_ID:
				{
					(can_count<=50) ? GetEncoderBias(&HeroReloadEncoder ,msg):EncoderProcess(&HeroReloadEncoder,msg);       //获取到编码器的初始偏差值            
				...
				case CAN_BUS2_HEROTRACK1_FEEDBACK_MSG_ID:
				{
					(can_count<=50) ? GetEncoderBias(&HeroTrackEncoder ,msg):EncoderProcess(&HeroTrackEncoder,msg);       //获取到编码器的初始偏差值            
				}break;
				/*case CAN_BUS2_SHOOTMOTOR_MSG_ID:
				{
					//LostCounterFeed(GetLostCounter(LOST_COUNTER_INDEX_MOTOR4));
...
/********************************************************************************
   给电调板发送指令，ID号为0x1FF，只用1个电调板，数据回传ID为0x207
*********************************************************************************/
void Set_ShootMotor_Speed(CAN_TypeDef *CANx, int16_t shoot_motor_iq)
{
    ...
    tx_message.Data[4] = (unsigned char)(shoot_motor_iq >> 8);
    tx_message.Data[5] = (unsigned char)shoot_motor_iq;
    tx_message.Data[6] = 0x00;
    tx_message.Data[7] = 0x00;
    CAN_Transmit(CANx,&tx_message);
}
